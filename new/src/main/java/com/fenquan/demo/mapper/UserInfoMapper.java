...
public interface UserInfoMapper extends BaseMapper<UserInfo> {
    ...
    @Select("select renyuan.id,renyuan.B,renyuan.C,renyuan.D,renyuan.E,renyuan.renyuan_id,renyuan.zhuangtai,renyuan.email,renyuan.phone,renyuan.bianhao,quanxian.id as quanxian_id from baitaoquanxian_renyun as renyuan left join baitaoquanxian_copy1 as quanxian on  renyuan.renyuan_id = quanxian.renyuan_id where renyuan.B=#{company}")
    List<UserInfo> queryC(String company);

    @Select("select renyuan.id,renyuan.B,renyuan.C,renyuan.D,renyuan.E,renyuan.renyuan_id,renyuan.zhuangtai,renyuan.email,renyuan.phone,renyuan.bianhao,quanxian.id as quanxian_id from baitaoquanxian_renyun as renyuan left join baitaoquanxian_copy1 as quanxian on  renyuan.renyuan_id = quanxian.renyuan_id where renyuan.B=#{company} and renyuan.C like '%'+#{query}+'%'")
    List<UserInfo> queryC_Inquire(String company ,String query);

    @Insert("insert into baitaoquanxian_renyun(quanxian,B,C,D,E,renyuan_id,zhuangtai,email,phone,bianhao) values(company,B,add_C,add_E,renyuan_id,add_zhuangtai,add_email,add_phone,add_bianhao)")
    List<UserInfocopy> add(String add_C, String add_D, String add_E, String add_zhuangtai, String add_email, String add_phone, String add_bianhao);
